...
20 year old me became fascenited by the vintage texture of _Them&I´s_ album covers — subtle grain, imperfect noise, a feeling that digital images often lack. That led to a question:

> Can I recreate film grain _procedurally_ using my GPU?
...
pipeline of the GPU, not the complete engine. That is, we can only render using rasterization.

Remember! We find two main rendering paradigms:
...
Important distinction: a vertex is a point in space, while a fragment is a _potential pixel_ (before visibility tests). For instance, imagine we want to render an apple hidden behind a wall. The apple vertices exist, as well as its fragments, but not its pixels for they are not rendered.
...
However, data is usually introduced through **UV coordinates**, whose range is `[0, 1]`. To obtain clip space from uv:

$$
x_{clip} = 2 \cdot x_{uv} - 1
$$

<div class="pt-2 flex flex-col max-w-[80%] md:flex-row gap-4 items-center justify-center" style="--coord-size: 320px;">
  <svg style="width: var(--coord-size); height: var(--coord-size);" viewBox="0 0 1518 1572" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect x="126" y="137" width="1321" height="1321" fill="var(--secondary)"/>
  <path d="M786 137C786 917.332 786 1251.05 786 1460" stroke="var(--text_2)" stroke-width="16"/>
  <path d="M1453 797C676.207 797 330.003 797 122 797" stroke="var(--text_2)" stroke-width="16"/>
  ...
  <svg style="width: var(--coord-size); height: var(--coord-size);" viewBox="0 0 1423 1467" fill="none" xmlns="http://www.w3.org/2000/svg">
